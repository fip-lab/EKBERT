...
kgclue_knowledge_address = '../../../src/data/raw/kgclue/knowledge.txt'
kgclue_s_po_address = '../../data/kgclue_s_po.json'

with open(kgclue_knowledge_address, 'r', encoding='utf-8') as f:
    kgclue_raw_triple = f.read().splitlines()
...
s_po = []
for item in tqdm(kgclue_raw_triple):
    list_temp = []
    item1 = item.split('\t')
    if len(item1) == 3:
        s = item1[0].replace(' ', '')
...
with open(kgclue_s_po_address, 'w', encoding='utf-8') as f:
    json.dump(final_s_po_list, f, ensure_ascii=False)

print('kgclue_know处理完毕！')