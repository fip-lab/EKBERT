...
with open(ccks2020_kbqa_test_knowques_compansw_new_inp_address, 'r', encoding='utf-8') as f:
    ccks2020_kbqa_test_knowques_compansw_new_inp = json.load(f)
# 加载训练好的模型和分词器
tokenizer = BertTokenizer.from_pretrained('../gpt2_trained_model/gpt2_trained_model_ccks2020')
model = GPT2LMHeadModel.from_pretrained('../gpt2_trained_model/gpt2_trained_model_ccks2020')
...
for text in tqdm(ccks2020_kbqa_test_knowques_compansw_new_inp):
#     if idx <20:
    # text = '知识:别名是浙东行署社会教育工作队,中文名是浙东行署社会教育工作队,类型是越剧演出团体,骨干是竹方森、竺方渭、金桂芳,筹建时间是1943年7月,问题:浙东行署社会教育工作队何时建筑的?'
    truncated_text = text[:1024]
    generated = generator(truncated_text, max_length=1024, num_return_sequences=1)[0]['generated_text']
    # print(generated)
    text_new = generated[len(text):].replace(' ', '')
    # print(text_new)
    total_answer.append(text_new)
# print(total_answer)
with open(ccks2020_kbqa_test_model_gene_answer_address, 'w', encoding='utf-8') as f:
