    ...
    for line_id, line in enumerate(sentences):
        ...
        try:
            if len(line) == 2:
                ...
                tokens, pos, vm, _ = kg.add_knowledge_with_vm([text], add_pad=True, max_length=args.seq_length, model_type=args.model_type)
                tokens = tokens[0]
                pos = pos[0]
                vm = vm[0].astype("bool")
...
def main(args):
    ...
    # parser.add_argument("--output_model_path", default="./models/classifier_model.bin", type=str, help="Path of the output model.")
    ...
    def evaluate(args, metrics='Acc'):
        ...
        if not args.mean_reciprocal_rank:
            ...
            # 读取数据标签
            if args.is_test:
                refer_path = os.path.join(file_path, 'data/processed/nlpcc_kbqa/ed_kbert/test.tsv')
            else:
                refer_path = os.path.join(file_path, 'data/processed/nlpcc_kbqa/ed_kbert/dev.tsv')
            # print('refer_path', refer_path)
            refer_df = pd.read_csv(refer_path, sep='\t', header=None, skiprows=1)
            if args.is_debug:
                # 测试部分案例
            ...
            print(len(total_question_list))
            print(len(total_gold_list))
    ...
    if args.do_eval:
        ...
        print('gpu数{}'.format(torch.cuda.device_count()))
        if torch.cuda.device_count() > 1:
