...
from train_code_no_temp.nlpcc import T5FineTuner, mt5Dataset
import torch
import json
import random
import numpy as np
...
model = torch.load('../flant5_trained_model_no_temp/mt5_nlpcc_no_temp.pkl')

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
t5_model = t5_model.to(device)
model = model.to(device)

nlpcc_kbqa_test_model_gene_answer_no_temp_address = '../flant5_model_gene_answer_no_temp/nlpcc_kbqa_test_model_gene_answer_no_temp.json'
nlpcc_kbqa_test_knowques_compansw_no_temp_address = '../../mt5/data_no_temp/nlpcc_kbqa_test_knowques_compansw.json'
...
with open(nlpcc_kbqa_test_model_gene_answer_no_temp_address, 'w', encoding='utf-8') as f:
