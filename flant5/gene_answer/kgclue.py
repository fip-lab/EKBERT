...
from train_code.kgclue import FLANT5FineTuner, flant5Dataset
import torch
import json
import random
import numpy as np
...
model = torch.load('../flant5_trained_model/mt5_kgclue.pkl')

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
t5_model = t5_model.to(device)
model = model.to(device)

kgclue_kbqa_test_model_gene_answer_address = '../flant5_model_gene_answer/kgclue_kbqa_test_model_gene_answer.json'
