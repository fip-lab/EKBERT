...
kglcue_kbqa_test_knowques_compansw_address = '../../../mt5/data/kgclue_test_data.json'
kglcue_kbqa_test_model_gene_answer_address = '../../bart_model_gene_answer/kgclue_kbqa_test_model_gene_answer_3.json'
...
def bleu_score(model_answer_address_test, comp_answ_address_test):
    ...
    for item in model_answer:
        model_answer_new.append(item.replace(' ', ''))
    ...
    # idx =idx + 1
    assert len(model_answer_new) == len(comp_answ_test_templist)
    for senten1, senten2 in zip(model_answer_new, comp_answ_test_templist):
    ...
    total_scores = total_scores / (len(model_answer_new))
    return total_scores


nlpcc_test_bleu_scores = bleu_score(kglcue_kbqa_test_model_gene_answer_address,
                                    kglcue_kbqa_test_knowques_compansw_address)
print('kglcue的bleu值为：', nlpcc_test_bleu_scores)
