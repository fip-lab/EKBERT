...
nlpcc_kbqa_test_knowques_compansw_address = '../../mt5/data/nlpcc2018_test_data.json'
nlpcc_kbqa_test_model_gene_answer_address = '../data/nlpcc2018_answer.txt'
...
def get_rouge_r_test(model_answer_address_test, comp_answ_address_test):
    with open(model_answer_address_test, 'r', encoding='utf-8') as f:
        model_answer_test_temp = f.readlines()
    model_answer_test = []
    for item in model_answer_test_temp:
        model_answer_test.append(item.replace('\n', ''))
    ...
    list = []
    for idx, item in enumerate(model_answer_test):
        if item == '':
            list.append(idx)
    for i in sorted(list, reverse=True):
        del model_answer_test[i]
        del comp_answ_test_list[i]
...
nlpcc_rouge1_r_scores, nlpcc_rouge2_r_scores, nlpcc_rouge3_r_scores = get_rouge_r_test(nlpcc_kbqa_test_model_gene_answer_address,
                                                                                  nlpcc_kbqa_test_knowques_compansw_address)
nlpcc_rouge1_f_scores, nlpcc_rouge2_f_scores, nlpcc_rouge3_f_scores = get_rouge_f_test(nlpcc_kbqa_test_model_gene_answer_address,
                                                                                  nlpcc_kbqa_test_knowques_compansw_address)
nlpcc_rouge1_p_scores, nlpcc_rouge2_p_scores, nlpcc_rouge3_p_scores = get_rouge_p_test(nlpcc_kbqa_test_model_gene_answer_address,
                                                                                  nlpcc_kbqa_test_knowques_compansw_address)
# print('平均rouge1_r分数为：', nlpcc_rouge1_r_scores)
print('nlpcc2018的平均rouge2_r分数为：', nlpcc_rouge2_r_scores)
# print('平均rouge3_r分数为：', nlpcc_rouge3_r_scores)
#
# print('平均rouge1_f分数为：', nlpcc_rouge1_f_scores)
# print('平均rouge2_f分数为：', nlpcc_rouge2_f_scores)
# print('平均rouge3_f分数为：', nlpcc_rouge3_f_scores)
#
# print('平均rouge1_p分数为：', nlpcc_rouge1_p_scores)
# print('平均rouge2_p分数为：', nlpcc_rouge2_p_scores)
# print('平均rouge3_p分数为：', nlpcc_rouge3_p_scores)