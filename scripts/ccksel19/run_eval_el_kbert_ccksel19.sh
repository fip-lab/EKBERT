...
CUDA_VISIBLE_DEVICES=${device} python run_kbert_ner.py \
  ...
  --is_test \
  --model_type='ner' \
  --output_predict_result_path="../../output/finetune_model/ccks_el_kbert_md/test_prediction.json" \
...
CUDA_VISIBLE_DEVICES=${device} python run_entity_linking.py \
  --model_name='kbert' \
  --load_md_predict_data_path='../output/finetune_model/ccks_el_kbert_md' \
  --dump_ed_predict_data_path='../src/data/processed/ccks_el_2019/ed_kbert' \
  --dataset='ccksel19'

cd kbert
## 读ed数据进行预测，得到el任务结果
CUDA_VISIBLE_DEVICES=${device} python run_kbert_cls.py \
  --pretrained_model_path="../../output/finetune_model/ccks_el_kbert_ed/kbert_ccksel19_ed.bin" \
  --config_path="../../pretrained_model/bert-base-chinese-google/google_config.json" \
  --vocab_path="../../pretrained_model/bert-base-chinese-google/google_vocab.txt" \
  --train_path="../data/processed/ccks_el_2019/ed_kbert/train.tsv" \
  --test_path="../data/processed/ccks_el_2019/ed_kbert/predict.tsv" \
  --kg_name="ccks_el_2019" \
  --do_eval \
  --is_test \
  --model_type='matching' \
  --output_model_path="../../output/finetune_model/ccks_el_kbert_ed/kbert_ccksel19_ed.bin" \
  --compute_pipeline_el_metric \
  --pipeline_el_md_dir='../../output/finetune_model/ccks_el_kbert_md' \
  --pipeline_el_ed_dir='../../output/finetune_model/ccks_el_kbert_ed'
