...
CUDA_VISIBLE_DEVICES=${device} python run_kbert_ner.py \
  ...
  --is_test \
  --model_type='ner' \
  --output_predict_result_path="../../output/finetune_model/nlpcc_kbqa_kbert_md/test_prediction.json" \
...
CUDA_VISIBLE_DEVICES=${device} python run_entity_linking.py \
  --model_name='kbert' \
  --load_md_predict_data_path='../output/finetune_model/nlpcc_kbqa_kbert_md' \
  --dump_ed_predict_data_path='../src/data/processed/nlpcc_kbqa/ed_kbert'

cd kbert
## 读ed数据进行预测，得到el任务结果
CUDA_VISIBLE_DEVICES=${device} python run_kbert_cls.py \
  --pretrained_model_path="../../output/finetune_model/nlpcc_kbqa_kbert/kbert_nlpcc_ed.bin" \
  --config_path="../../pretrained_model/bert-base-chinese-google/google_config.json" \
  --vocab_path="../../pretrained_model/bert-base-chinese-google/google_vocab.txt" \
  --train_path="../data/processed/nlpcc_kbqa/ed_kbert/train.tsv" \
  --test_path="../data/processed/nlpcc_kbqa/ed_kbert/predict.tsv" \
  ...
  --pipeline_el_md_dir='../../output/finetune_model/nlpcc_kbqa_kbert_md' \
  --pipeline_el_ed_dir='../../output/finetune_model/nlpcc_kbqa_kbert'
