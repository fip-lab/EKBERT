...
CUDA_VISIBLE_DEVICES=${device} python run_kbert_ner.py \
 ...
 --batch_size=8 \
 --model_type='ner' \
 --output_predict_result_path="../../output/finetune_model/ccks_kbqa_kbert_md/test_prediction.json" \
...
CUDA_VISIBLE_DEVICES=${device} python run_entity_linking.py \
  --model_name='kbert' \
  --load_md_predict_data_path='../output/finetune_model/ccks_kbqa_kbert_md' \
  --dump_ed_predict_data_path='../src/data/processed/ccks_kbqa_2020/ed_kbert' \
  --dataset='cckskbqa20'

cd kbert
# ## 读ed数据进行预测，得到el任务结果
CUDA_VISIBLE_DEVICES=${device} python run_kbert_cls.py \
  --pretrained_model_path="../../output/finetune_model/ccks_kbqa_kbert_ed/ccks_kbqa_kbert_ed.bin" \
  --config_path="../../pretrained_model/bert-base-chinese-google/google_config.json" \
  --vocab_path="../../pretrained_model/bert-base-chinese-google/google_vocab.txt" \
  --train_path="../data/processed/ccks_kbqa_2020/ed_kbert/train.tsv" \
  --test_path="../data/processed/ccks_kbqa_2020/ed_kbert/predict.tsv" \
  --kg_name="ccks_kbqa_2020" \
  --do_eval \
  --batch_size=8 \
  --is_test \
  ...
  --pipeline_el_md_dir='../../output/finetune_model/ccks_kbqa_kbert_md' \
  --pipeline_el_ed_dir='../../output/finetune_model/ccks_kbqa_kbert_ed'
